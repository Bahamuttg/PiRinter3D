...
void MotorConfigDialog::WriteCfgFile()
{
    ...
    if(MotorCfg.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate))
    {
        ...
        OutStream << ui->chkNOTX->isChecked() << ";" << ui->chkEnableX->isChecked() << ";" << ui->rbFullX->isChecked() << ";" << ui->rbHalfX->isChecked() << ";" << ui->dsbResX->value()  << ";" << ui->seMinDelayX->value() << "\n";

        OutStream << "MotorConfig::YAxis;" << ui->leA1_Y->text() << ";" << ui->leA2_Y->text() << ";" << ui->leB1_Y->text() << ";" << ui->leB2_Y->text() << ";" << ui->leEnableY->text() << ";";
        OutStream << ui->chkNOTY->isChecked() << ";" << ui->chkEnableY->isChecked() << ";" << ui->rbFullY->isChecked() << ";" << ui->rbHalfY->isChecked() << ";" << ui->dsbResY->value() << ";" << ui->seMinDelayY->value()   <<"\n";

        OutStream << "MotorConfig::ZAxis;" << ui->leA1_Z->text() << ";" << ui->leA2_Z->text() << ";" << ui->leB1_Z->text() << ";" << ui->leB2_Z->text() << ";" << ui->leEnableZ->text() << ";";
        OutStream << ui->chkNOTZ->isChecked() << ";" << ui->chkEnableZ->isChecked() << ";" << ui->rbFullZ->isChecked() << ";" << ui->rbHalfZ->isChecked() << ";" << ui->dsbResZ->value()<< ";" << ui->seMinDelayZ->value()   <<"\n";

        OutStream << "MotorConfig::ExtAxis;" << ui->leA1_EX->text() << ";" << ui->leA2_EX->text() << ";" << ui->leB1_EX->text() << ";" << ui->leB2_EX->text() << ";" << ui->leEnableEX->text() << ";";
        OutStream << ui->chkNOTEX->isChecked() << ";" << ui->chkEnableEX->isChecked() << ";" << ui->rbFullEX->isChecked() << ";" << ui->rbHalfEX->isChecked() << ";" << ui->dsbResEX->value() << ";" << ui->seMinDelayExt->value()  <<"\n";
        OutStream.flush();
        MotorCfg.close();
    }
    else
    ...
    if(MotorCfg.open(QIODevice::ReadOnly | QIODevice::Text ))
    {
        ...
        while(!InStream.atEnd())
        {
            ...
            if(Line.contains("MotorConfig"))
            {
                ...
                if(Params[0].contains("XAxis") && Params.count() == 12)
                {
                ...
                if(Params[0].contains("YAxis") && Params.count() == 12)
                {
                ...
                if(Params[0].contains("ZAxis") && Params.count() == 12)
                {
                ...
                if(Params[0].contains("ExtAxis") && Params.count() == 12)
                {
